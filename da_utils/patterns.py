...
def analyze_missing_patterns(df: pd.DataFrame):
    ...
    # 3. 결측값 조합 패턴  -> 결측이 동시에 발생하는 변수들 파악 -> 유의미하게 많으면 연관을 추정 -> 그룹별 처리 전략
    # 결측값이 있는 컬럼들만 T/F 표시
    missing_pattern = df[df.columns[df.isna().any()]].isna() # 하나라도 비어있는 컬럼들을 마스킹해서 그 비어있는 칼럼들만 데이터프레임 묶어서 다시 비어있는 걸 확인
